...
extern fifo_rx_def fifo_usart_rx_6;
fifo_rx_def *pfifo_referee = &fifo_usart_rx_6;
...
/**
  ...
  */
uint8_t referee_read_data(void)
{
                    ...
                    if (Verify_CRC16_Check_Sum(buff_read + i, judge_length) == 1)
                    {
                        ...
                        switch (Referee.Cmd_ID)
                        {
                            ...
                            //							break;
                            						...
                            						case 0x0003:  //机器人血量数据
                            							memcpy(&Referee.RobotHP, (buff_read + i + DATA), LEN_game_robot_survivors);
                            							break;
...
uint16_t referee_red_outpost_HP(void)			//红方前哨站血量
{
    return (Referee.RobotHP.red_outpost_HP);
}
uint16_t referee_blue_outpost_HP(void)		//蓝方前哨站血量
{
    return(Referee.RobotHP.blue_outpost_HP);
}
