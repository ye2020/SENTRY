...
/* 测试一个数字是否是 2 的幂 */
/* 进行位与操作 将数转化为二进制 如果一个数是2的n次方，那么这个数用二进制表示时其最高位为1，其余位为0 */
...
uint32_t roundup_pow_of_two(uint32_t Num)
{
    ...
    if (IS_POWER_OF_2(Num) || Num == 0)
        return Num;
    else if (Num > LONG_MAX)						// LONG_MAX: 0x7fffffffL (0111 1111 1111 1111 1111 1111 1111 1111)
...
uint32_t fifo_read_buff(fifo_rx_def *pfifo, uint8_t *buffer, uint32_t len)
{
    ...
    len = min_fifo(len, pfifo->in - pfifo->out); //获取队列in到out数组的大小
    ...
    memcpy(buffer, pfifo->buffer + (pfifo->out & (pfifo->size - 1)), length);// 写入缓冲区

    /* then get the rest (if any) from the beginning of the buffer */
    memcpy(buffer + length, pfifo->buffer, len - length);                  // 然后从缓冲区的开头获取剩余的(如果有的话)
...
uint32_t fifo_write_buff(fifo_rx_def *pfifo, uint8_t *buffer, uint32_t len)
{
    ...
    len = min_fifo(len, (pfifo->size - (pfifo->in - pfifo->out)));							// 获取除去 out 到 in 之外的数组大小
