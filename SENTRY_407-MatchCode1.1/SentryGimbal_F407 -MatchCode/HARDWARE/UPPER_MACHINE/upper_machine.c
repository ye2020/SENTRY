...
void pid_parameter_receive(PidTypeDef *pid_speed, PidTypeDef *pid_position)
{
    ...
    uint32_t length = fifo_read_buff(pfifo_2, buff_read, sizeof(buff_read));				// 读取数据写入环形缓冲区
    if (length)
    {
        if ((uint8_t *)strstr((char *)buff_read, (char *)"nep p_Kp:"))							// 在从循环缓冲区写入的目标缓冲区中查找对应字符串“nep p_Kp:”
        {
